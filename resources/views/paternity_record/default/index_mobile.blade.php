...
@section('content')
    <div class="m-portlet m-portlet--mobile">
        <form method="get" action="{{route('paternity_records.index',['mid'=>request('mid')])}}"
            ...
            <div class="m-portlet__body ">
                ...
                <div class=" m-form__group row align-items-center">
                    ...
                    <div class="col-lg-4">
                        <div class="form-group">
                            <select class="form-control" id="status" name="is_handle">
                                {!! is_handle_select(request('is_handle')) !!}
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-lg-4">
                        <div class="form-group">
                            ...
                            <a href="{{ route('paternity_records.create',['mid'=>'6c8112484593862335c4644c0c8932df']) }}"
                ...
                <div class="m-widget-body">
                    <div class="m-section m-section-none">
                        <h3 class="m-section__heading m-line-height-25">
                            <a href="{{route('paternity_records.show',[
                            'paternity_record'=>$malfunction->id,'mid'=>request('mid')
                            ])}}" class="action-show">
                                {{str_limit($malfunction->question,50)}}
                            </a>
                        </h3>
                        <span class="m-section__sub m-section__sub-margin-bottom-none">
                            所属项目：{{$malfunction->project ? $malfunction->project->title : null}} <br>
                            记录日期：{{$malfunction->created_at}} <br>
                            记录人：{{$malfunction->user ? $malfunction->user->name : null }}
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div class="btn-group m-btn-group col" role="group" aria-label="">
                ...
                || $malfunction->user_id == get_current_login_user_info()
                 || check_project_owner($malfunction->project,'company'))
                    <a href="{{route('paternity_records.edit',['malfunction'=>$malfunction->id,'mid'=>request('mid')])}}"
                    ...
                    col m-btn--icon-center action-delete " data-malfunction-id="{{$malfunction->id}}"
                       href="{{route('paternity_records.destroy',['malfunction'=>$malfunction->id,'mid'=>request('mid')])}}">
                        <span>
