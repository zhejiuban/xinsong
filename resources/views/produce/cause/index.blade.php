		...
		<div class="m-form m-form--label-align-right  m--margin-bottom-20">
			<div class="row align-items-center">
				...
				<div class="col-xl-4 order-1 order-xl-2 m--align-right">
					<a href="{{ menu_url_format(route('fault_causes.create'),['mid'=>request('mid')]) }}" data-toggle="modal" data-target="#m_role_modal" class="btn btn-sm btn-primary m-btn m-btn--icon m-btn--air m-btn--pill">
						<span>
    ...
    var DataList = function () {
      ...
      datatable = $('.m_datatable').mDatatable({
        ...
        columns: [{
          ...
          overflow: 'visible',
          template: function (row) {
						var del = '<a href="'+mAppExtend.laravelRoute('{{route_uri("fault_causes.destroy")}}',{fault_cause:row.id })+'" class="action-delete m-portlet__nav-link btn m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill" title="删除"><i class="la la-trash"></i></a>';
						if(row.is_system == 1){ del = ''};
            return '<a href="'+mAppExtend.laravelRoute('{{route_uri("fault_causes.edit")}}',{fault_cause:row.id})+'" class="action-edit m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="编辑">\
